...
// [4] 닭 클래스 , 닭클래스는 조류 클래스로부터 상속받기
class 닭 extends 조류 {
	닭(){
		myName = "닭";
		System.out.println("[[ 닭 생성자 탄성 ]]");
	}
}
...
public class Example1 {
	public static void main(String[] args) {
		...
		Object obj2 = sparrow;	// 참새 타입의 자료(객체) 가 Object 타입으로 변환이 된다. ( 자동타입변환 )
			...
			// - 동물은 조류 과 참새가 될 수 없다.
			// 즉] 부모 타입의 자료는 자식 타입으로 변환이 될 수 없다. 부모객체는 자식타입을 참조하지 않는다.
			// 오류 : class day15.step1.동물 cannot be cast to class day15.step1.참새 (day15.step1.동물 and day15.step1.참새 are in unnamed module of loader 'app')
		// 참새 sparrow3 = (참새)animal;	// 동물 타입의 자료(객체) 가 참새 타입으로 변환이 안된다.
		// 조류 bird3 = (조류)animal; 		// 동물 타입의 자료(객체) 가 조류 타입으로 변환이 안된다.
		
			// * 캐스팅 * : 가능한 전제조건은 본래 하위 타팁의 객체가 상위 타입에서 다시 본래 하위 타입 변환 허용.
			// 즉] 부모 타입의 자료는 자식 타입으로 변환이 될 수 없지만 단, 객체가 본래 자식 타입의 객체 이였다면 가능하다.
		동물 animal4 = sparrow; 
			// 참새 타입의 객체 를 동물 타입으로 변환
		참새 sparrow4 = (참새)animal4; 
			// 동물 타입은 참새 타입으로 변환 가능 ,단 객체가 본래 자식 타입 이였다면.
			
			// 형제간의 타입변환이 불가능하다.
		닭 chicken1 = new 닭(); // 인스턴스(4개 생성) 닭 , 조류 , 동물 , Object
		// 참새 sparrow5 = (참새)chiken1; // 닭은 참새를 모른다.
