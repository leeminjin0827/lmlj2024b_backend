...
insert into employees( employee_1d , first_name , last_name , email , hire_date , job_id )
...
insert into products( produck_id , product_name , category , price , stock_quantity )
...
select * from procucts where price = 1000;

[문제5]: employees 테이블에서 employee_id가 101인 직원의 email을 'john.doe@company.com'으로 업데이트하세요.

update employees set employee_id  = 101 where email = 'jogn.doe@company.com';

[문제6]: products 테이블에서 product_id가 201인 제품의 price를 24.99로 업데이트하세요.

update products set product_id = 201 where price = 24.99;
